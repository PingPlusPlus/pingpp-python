...
'''
    ...
    创建charge对象-cb_alipay渠道
'''
print "创建charge对象:"
try:
    charge = pingpp.Charge.create(
        order_no='12334567890',  # 推荐使用 8-20 位，要求数字或字母，不允许其他字符
        amount=100,  # 订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填 100）
        app=dict(id=app_id),
        channel='cb_alipay',  # https://www.pingxx.com/api#支付渠道属性值
        currency='usd',
        client_ip='127.0.0.1',  # 发起支付请求客户端的 IP 地址，格式为 IPV4，如: 127.0.0.1
        subject='Your Subject',
        body='Your Body',
        extra=dict(
            # split_fund_info :分账列表 account: 接受分账资金的支付宝账户ID。amount: 分账的金额。desc 分账描述信息。
            split_fund_info=[
                dict(
