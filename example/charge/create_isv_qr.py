...
try:
    charge = pingpp.Charge.create(
        ...
        body='Your Body',
        extra=dict(
            pay_channel='alipay',  # 必须，支付具体渠道，目前支持：alipay、wx、bfb。
            terminal_id='SH008',  # 必须，终端号，要求不同终端此号码不一样，会显示在对账单中，如A01、SH008等。
            # 可选，商品列表，上送格式参照下面示例。
            # 字段解释：goods_id:商户定义商品编号（一般商品条码）
            #     unified_goods_id:统一商品编号(可选)，goods_name:商品名称，goods_num:数量
            #     price:单价(单位分)，goods_category:商品类目(可选)，body:商品描述信息(可选)，
            #     show_url:商品的展示网址(可选)
            goods_list=[
                dict(
